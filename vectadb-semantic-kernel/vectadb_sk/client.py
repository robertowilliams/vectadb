...
Mirrors the client in vectadb-ag2 and vectadb-langchain but scoped to
Semantic Kernel integration (source system = "semantic_kernel").
"""
...
class VectaDBClient:
    ...
    async def is_healthy(self) -> bool:
        """Return True if VectaDB is reachable and healthy."""
        try:
    ...
    async def ingest_events_bulk(
        ...
        *,
        ...
        logger.debug("Ingesting %d events in %d batch(es)", len(events), len(batches))
    ...
    async def search(
        ...
        *,
        ...
        Returns a list of VectorSearchResult ordered by descending score.
        """
        ...
        try:
            resp = await self._client.post("/api/v1/query/hybrid", json=payload)
            self._raise_for_status(resp)
            data = resp.json()
            ...
            raise VectaDBClientError(f"Connection error during search: {exc}") from exc
    ...
    async def _send_bulk_with_retry(
        ...
        *,
        ...
        return BulkIngestionResponse(ingested=0, failed=batch_size, errors=errors)
...
# ---------------------------------------------------------------------------
# Synchronous wrapper (for non-async Semantic Kernel usage)
# ---------------------------------------------------------------------------
...
class SyncVectaDBClient:
    ...
    Allows VectaDB ingestion from synchronous callers (e.g. sync kernel functions
    or plugin methods) without blocking the main event loop.
    """
    ...
    def search(self, query: str, **kw: Any) -> list[VectorSearchResult]:
        try:
            return self._run(self._async_client.search(query, **kw))  # type: ignore[union-attr]
        except Exception as exc:

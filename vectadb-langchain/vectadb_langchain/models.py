"""
VectaDB data models for LangChain audit events.

Maps LangChain lifecycle events to VectaDB's event ingestion schema,
...
class LangChainEventType(str, Enum):
    """Typed event taxonomy aligned with VectaDB's ontology for LangChain runs."""
...
class AgentRunState(BaseModel):
    """Tracks mutable state for a single agent within a run."""
    agent_id: str
    role: str
    session_id: str
    trace_id: Optional[str] = None
...
class RunState(BaseModel):
    """Top-level state for a complete LangChain tracer session."""
    session_id: str
    run_name: str
    started_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))

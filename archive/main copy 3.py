# main.py
# ShortID / Agent Registry System using CouchDB, Weaviate, and Nebula Graph
...
try:
    ...
    if not ok:
        ...
        # Create space, tags, and edges if not exist
        with nebula_pool.session_context(NEBULA_USER, NEBULA_PASSWORD) as session:
            ...
            if not res.is_succeeded():
                logger.warning(f"[Nebula] Failed to create/use space: {res.error_msg()}")
            ...
            session.execute(
                ...
                "shortid string, role string, goal string, metadata string);"
            )
            session.execute(
                "CREATE TAG IF NOT EXISTS Task("
                "shortid string, task_name string, description string, priority string, metadata string);"
            )
...
def nebula_upsert_agent(agent_id: str, meta: Dict[str, Any]) -> None:
    ...
    try:
        ...
        with nebula_pool.session_context(NEBULA_USER, NEBULA_PASSWORD) as session:
            session.execute(f"USE {NEBULA_SPACE};")
            nql = (
                'INSERT VERTEX IF NOT EXISTS Agent(shortid, role, goal, metadata) '
                f'VALUES "{_nebula_escape(agent_id)}":('
                f'"{_nebula_escape(agent_id)}",'
                f'"{_nebula_escape(role)}",'
                f'"{_nebula_escape(goal)}",'
...
def nebula_upsert_task(task_id: str, meta: Dict[str, Any]) -> None:
    ...
    try:
        ...
        with nebula_pool.session_context(NEBULA_USER, NEBULA_PASSWORD) as session:
            session.execute(f"USE {NEBULA_SPACE};")
            nql = (
                'INSERT VERTEX IF NOT EXISTS Task(shortid, task_name, description, priority, metadata) '
                f'VALUES "{_nebula_escape(task_id)}":('
                f'"{_nebula_escape(task_id)}",'
                f'"{_nebula_escape(task_name)}",'
                f'"{_nebula_escape(description)}",'
                f'"{_nebula_escape(priority)}",'
                f'"{_nebula_escape(metadata_json)}");'
            )
            res = session.execute(nql)
            if not res.is_succeeded():
...
async def handle_json_rpc(
    ...
    # For tasks, require agent_id and check existence in Nebula

...
try:
    ...
    if not ok:
        ...
        with nebula_pool.session_context(NEBULA_USER, NEBULA_PASSWORD) as session:
            ...
            if not res.is_succeeded():
                logger.warning(
                    f"[Nebula] Failed to switch to space {NEBULA_SPACE}: {res.error_msg()}"
                )
...
def nebula_upsert_agent(agent_id: str, meta: Dict[str, Any]) -> None:
    """
    Upsert an Agent vertex in Nebula.
    """
    if not nebula_pool:
        return
    try:
...
def nebula_upsert_task(task_id: str, meta: Dict[str, Any]) -> None:
    """
    Upsert a Task vertex in Nebula.
    """
    if not nebula_pool:
        return
    try:
...
def nebula_link_task_to_agent(task_id: str, agent_id: str) -> None:
    """
    Link a Task vertex to an Agent vertex via BELONGS_TO edge.
    """
    if not nebula_pool:
        return
    try:
...
def _nebula_list_vertices(tag: str, limit: int = 100) -> List[Dict[str, Any]]:
    """
    Return vertices of a given tag (Agent/Task) from Nebula.
    We only read and serialize the row values as strings for inspection.
    """
    if nebula_pool is None:
        raise HTTPException(status_code=503, detail="Nebula Graph not available")

    try:
        with nebula_pool.session_context(NEBULA_USER, NEBULA_PASSWORD) as session:
            use_res = session.execute(f"USE {NEBULA_SPACE};")
            if not use_res.is_succeeded():
                logger.warning(f"[Nebula] USE {NEBULA_SPACE} failed: {use_res.error_msg()}")
                raise HTTPException(status_code=500, detail="Nebula space not available")

            nql = (
                f"MATCH (v:{tag}) "
                f"RETURN id(v) AS id, v.shortid AS shortid, v.metadata AS metadata "
                f"LIMIT {limit};"
            )
            res = session.execute(nql)
            if not res.is_succeeded():
                logger.warning(f"[Nebula] Query failed: {res.error_msg()}")
                raise HTTPException(status_code=500, detail="Nebula query failed")
            ...
            for row in res.rows():
                ...
                for idx, col_name in enumerate(keys):
                    try:
                        ...
                        row_dict[col_name] = str(row.values[idx])
        ...
        logger.warning(f"[Nebula] Read error: {e}")
        raise HTTPException(status_code=500, detail="Nebula read failed")
...
# ---------------------------------------------------------------
# NEBULA READ-ONLY ENDPOINTS
# ---------------------------------------------------------------


@app.get("/nebula/agents")
async def nebula_agents(
    x_api_key: str = Header(...),
    limit: int = 100,
):
    """
    List Agent vertices from Nebula Graph.
    """
    authenticate_request(x_api_key)
    items = _nebula_list_vertices("Agent", limit=limit)
    return {"count": len(items), "items": items}


@app.get("/nebula/tasks")
async def nebula_tasks(
    x_api_key: str = Header(...),
    limit: int = 100,
):
    """
    List Task vertices from Nebula Graph.
    """
    authenticate_request(x_api_key)
    items = _nebula_list_vertices("Task", limit=limit)
    return {"count": len(items), "items": items}

...
from fastapi import FastAPI, Header, HTTPException, Depends, Cookie, Request
...
AGENT_SIMILARITY_THRESHOLD = float(os.getenv("AGENT_SIMILARITY_THRESHOLD", 0.65))
AGENT_SIMILARITY_LIMIT = int(os.getenv("AGENT_SIMILARITY_LIMIT", 10))

API_SECRET_KEY = os.getenv("API_SECRET_KEY", "admin")
...
def verify_access_token(token: str) -> Optional[Dict[str, Any]]:
    """
    Verify token and return decoded payload, or None if invalid/expired.
    """
    try:
        decoded = jwt.decode(
            token,
...
@app.get("/auth/me")
async def auth_me(user = Depends(require_token)):
...
async def ui_couch_agents(user = Depends(require_token), limit: int = 100, skip: int = 0):
    """
...
@app.post("/ui/agents")
async def ui_create_agent(body: AgentRequest, user = Depends(require_token)):
    """
...
@app.post("/ui/tasks")
async def ui_create_task(body: TaskRequest, user = Depends(require_token)):
...
@app.get("/ui/graph")
async def ui_graph(user = Depends(require_token), limit: int = 200):
    """

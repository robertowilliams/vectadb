...
# ---------------------------------------------------------------
# WEAVIATE SETUP (correct text2vec-transformers configuration)
# ---------------------------------------------------------------
...
try:
    ...
    # Helper function to create a vectorized collection
    def create_vectorized_collection(name: str, description: str):
        if client.collections.exists(name):
            logger.info(f"Weaviate collection '{name}' already exists.")
            return

        logger.info(f"Creating vectorized collection '{name}'")
        ...
        client.collections.create(
            ...
            # CORRECT vectorizer config for new API
            vectorizer="text2vec-transformers",
            vector_index_config=weaviate.classes.config.VectorIndexConfig.hnsw(
                distance="cosine"
            )
        )


    # Create both collections with vectorizer enabled
    create_vectorized_collection(WEAVIATE_CLASS_NAME_AGENT, "Stores agent metadata")
    create_vectorized_collection(WEAVIATE_CLASS_NAME_TASK, "Stores task metadata")
...
def weaviate_find_similar_agents_from_metadata(
    ...
    """
    Given an agent metadata dict, query Weaviate for semantically similar agents.

    Uses v4 collections API with near_text, and returns only those with
    similarity >= threshold (where similarity = 1 - distance).
    """
    if client is None:
        logger.warning("Weaviate client not available; skipping similarity search")
        return []

    # Build a text query from the agent metadata
    text_parts: List[str] = []
    ...
    try:
        coll = client.collections.get(WEAVIATE_CLASS_NAME_AGENT)
...
async def embed_text(req: EmbedRequest, x_api_key: str = Header(...)):
    """
    Produce a text embedding using Weaviate's vectorizer.
    ...
    try:
        # Use any existing collection for embedding (metadata collections work fine)
        coll = client.collections.get(WEAVIATE_CLASS_NAME_AGENT)

        # Call Weaviate v4 "generate.embeddings()"
        result = coll.generate.embeddings(text=req.text)

        vector = result.get("vector")

        if not vector:
            raise RuntimeError("Weaviate returned no embedding vector")

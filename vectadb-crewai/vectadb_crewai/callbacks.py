...
CrewAI agents internally use LangChain's executor/chain machinery, which fires
a rich set of callback events at every step. This handler intercepts those
events and forwards them as typed VectaDB audit events, capturing:
...
class VectaDBCallbackHandler(BaseCallbackHandler):
    """
    LangChain callback handler that captures CrewAI agent runs into VectaDB.

    This class is not called directly by users — it is instantiated and
    managed by VectaDBTracer. It maintains a thread-safe event queue that
    VectaDBTracer flushes on completion.
    """
    ...
    def _make_event(
        ...
        return VectaDBEvent(
            ...
            source=VectaDBSource(
                system="crewai",
                log_group=f"crewai/{self.agent_id or 'unknown'}",
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        event_type = (
            CrewEventType.RETRIEVAL_START
            if "retriev" in chain_type.lower()
            else CrewEventType.CHAIN_START
        )

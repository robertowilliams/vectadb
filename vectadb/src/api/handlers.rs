...
/// Search entities by type and optional property filters.
///
...
///   "filter": { "session_id": "conv-abc" },
///   "limit": 100
/// }
/// ```
///
/// Returns all entities whose `entity_type` matches AND whose `properties`
/// contain every key-value pair in `filter` (exact equality).
pub async fn search_entities(
    ...
    let all_entities = surreal
        .query_entities(&request.entity_type)
        .await
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse::new(
                    "DatabaseError",
                    format!("Failed to search entities: {}", e),
                )),
            )
        })?;

    // Apply property filters (exact equality on each key-value pair)
    let limit = request.limit.unwrap_or(1000);
    let matching: Vec<EntityResponse> = all_entities
        .into_iter()
        .filter(|entity| {
            request.filter.iter().all(|(key, expected)| {
                entity
                    .properties
                    .get(key)

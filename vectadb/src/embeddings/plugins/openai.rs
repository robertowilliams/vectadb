...
impl OpenAIPlugin {
    ...
    async fn make_request(&self, input: EmbeddingInput) -> Result<OpenAIEmbeddingResponse> {
        let config = self
            .config
            .as_ref()
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| VectaDBError::Embedding(format!("OpenAI API request failed: {}", e)))?;
        ...
        let result: OpenAIEmbeddingResponse = response
            .json()
            .await
            .map_err(|e| VectaDBError::Embedding(format!("Failed to parse OpenAI response: {}", e)))?;

        // Update stats
        if let Ok(mut stats) = self.stats.write() {
            stats.total_requests += 1;
            stats.total_tokens += result.usage.total_tokens;
            stats.total_embeddings += result.data.len() as u64;
...
#[async_trait]
impl EmbeddingPlugin for OpenAIPlugin {
    ...
    async fn initialize(&mut self, config: PluginConfig) -> Result<()> {
        match config.provider {
            ...
            _ => Err(VectaDBError::InvalidInput(
                "Invalid provider config for OpenAI plugin".to_string(),
            )),
        }
    }

    async fn embed(&self, text: &str) -> Result<Vec<f32>> {
        let response = self.make_request(EmbeddingInput::Single(text.to_string())).await?;
    ...
    async fn embed_batch(&self, texts: &[String]) -> Result<Vec<Vec<f32>>> {
        ...
        let response = self.make_request(EmbeddingInput::Batch(texts.to_vec())).await?;
...
mod tests {
    ...
    async fn test_plugin_initialization() {
        ...
        assert!(result.is_ok());
        assert_eq!(plugin.dimension(), 1536);
        assert_eq!(plugin.max_batch_size(), 100);
    }
}
